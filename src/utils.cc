    ...
    namespace Utils {
        ...
        string to_string(const ::pEp_identity& ident, bool full, int indent)
        {
            ...
            if (full) {
                ...
                builder << std::string(indent, '\t') << "lang: '" << static_cast<string>(ident.lang)
                        << "'" << endl;
                ...
                builder << std::string(indent, '\t') << "enc_format: " << ident.enc_format << endl;
        ...
        std::string to_string(const ::message& msg, bool full, int indent)
        {
            ...
            builder << std::string(indent, '\t') << "from: "
                    << (msg.from != nullptr ? to_string(*msg.from, full, indent) : "NULL") << endl;
            builder << std::string(indent, '\t')
                    << "to: " << (msg.to != nullptr ? to_string(*msg.to, full, indent) : "NULL")
                    << endl;
            ...
            builder << std::string(indent, '\t') << "id: '" << (msg.id != nullptr ? msg.id : "NULL")
                    << "'" << endl;
            builder << std::string(indent, '\t') << "opt_fields: "
                    << (msg.opt_fields ? to_string(*msg.opt_fields, full, indent) : "NULL") << endl;
        ...
        string to_string(const ::pEp_group& group, bool full, int indent)
        {
            ...
            indent++;
            builder << std::string(indent, '\t')
                    << "group_identity: " << to_string(*group.group_identity, full, indent) << endl;
            builder << std::string(indent, '\t')
